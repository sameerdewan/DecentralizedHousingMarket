...
contract Ownable {
    ...
    constructor() public {
        _owner = msg.sender;
        emit NEW_OWNER(_owner);
    }
    //  3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
    modifier onlyOwner() {
        require(msg.sender == _owner, 'Error: Not owner.');
        _;
    }
    //  4) fill out the transferOwnership function
    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event NEW_OWNER(address newOwner);

    function transferOwnership(address newOwner) public onlyOwner {
        // TODO add functionality to transfer control of the contract to a newOwner.
        // make sure the new owner is a real address
        require(newOwner != address(0), 'Error: Invalid Address.');
        require(!Address.isContract(newOwner), 'Error: Cannot transfer ownership to Contract.');
        _owner = newOwner;
        emit NEW_OWNER(_owner);
    }
}
...
contract Pausable is Ownable {
    ...
    function setPaused(bool pauseBool) public onlyOwner {
        _paused = pauseBool;
        if (_paused == true) {
    ...
    modifier whenNotPaused() {
        require(_paused == false, 'Error: Paused.');
        _;
    }
    modifier paused() {
        require(_paused == true, 'Error: Not Paused');
        _;
    }
